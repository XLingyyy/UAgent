...
C13E1 content and repository checkpoint are accepted and published; the accidental external-only commit was removed from `main`, local external files remain intact and ignored, and the corrected remote was verified without divergence. No progression decision is made for MVP15 because the product-UI lifecycle and plugin provenance/fingerprint blockers remain. MVP16 implementation stays prohibited; only research and planning are allowed until a later supervisor checkpoint closes G13 and G16 and formally re-accepts MVP15.
