...
budget from 180 to 600 seconds. The same task remains active and all predecessor
live evidence is source-invalidated.
...
| Live Acceptance Resume 15          | Close the rendered MCP and exact-six contract defects discovered across the same live task.                                               | IN_PROGRESS        | Fifth interim source checkpoint `Review Verdict: PASS`; current implementation commit `bb89126d82416f0958050405ff1ab693505614f7`. The bounded timeout regression, workspace `1684`, typecheck, lint and web build pass; all earlier live artifacts are source-invalidated.                                                                 | Continue the same task with a fresh full live chain from this checkpoint.                     |
| D13 / 15A                          | Clean build/install/load and authoritative manifest identity.                                                                            | DISPATCHED         | Historical attempts are source-invalidated. Current implementation commit `bb89126d82416f0958050405ff1ab693505614f7` exists and the clean restart is dispatched.                                                                                                                                                 | Use that commit with final build/manifest tooling and verify installed == loaded == manifest. |
...
- D13 / 15A: `DISPATCHED` from `bb89126d82416f0958050405ff1ab693505614f7`.
...
- Recovery continues in the newly dispatched clean-checkout compatibility task
  ...
  `bb89126d82416f0958050405ff1ab693505614f7`. Real mutation remains gated until
