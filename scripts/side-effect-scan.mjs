...
const SCAN_DIRS = ["packages", "apps", "scripts", "tools", "docs", "integrations"];
...
const MVP15_AUTHORITY_FILES = [
  ...
  "integrations/unreal/UAgentAssetTools/Source/UAgentAssetTools/Private/UAgentAssetToolsModule.cpp",
  "integrations/unreal/UAgentAssetTools/Source/UAgentAssetTools/Private/UAgentAssetToolsContract.cpp",
];
...
function isTitleBarWindowInternals(rel, line) {
  return (
    /apps\/desktop\/web\/src\/shell\/TitleBar/.test(rel) &&
...
function isMvp15SaveAllEvidenceSummary(rel, line) {
  return (
    /packages\/runtime\/src\/mvp15-asset-changeset/.test(rel) &&
...
const CATEGORIES = [
  {
    ...
    description:
      "Provider wire API, auth, and secret terms in UI/state code (excludes docs/tests/contracts)",
    patterns: [
  ...
  },
  {
    ...
    description:
      "OS-level shell, filesystem, and process execution capability (excludes UI shell/browser labels)",
    patterns: [
  ...
  },
  {
    ...
    blockWhen: [
      (rel) =>
        /apps\/desktop\/web\/src\/(composer|settings|workspace|inspector|sidebar|app|components)\//.test(
          rel,
        ),
    ],
  },
  {
    ...
    description:
      "React product UI must not directly open browser/storage/clipboard/project side effects",
    patterns: [
    ...
    blockWhen: [
      (rel) =>
        /apps\/desktop\/web\/src\/(composer|settings|workspace|inspector|sidebar|shell|app|components)\//.test(
          rel,
        ),
    ],
  },
  {
    ...
    description:
      "React UI must not directly scan local files, call Tauri project commands, or keep raw paths",
    patterns: [
      { re: /@tauri-apps\/api/gi, label: "Tauri API import" },
      {
        re: /\binvoke\s*\(\s*["'](validate_project_root|scan_project_index|preview_project_file)/gi,
        label: "Tauri project command",
      },
    ...
    blockWhen: [
      (rel) =>
        /apps\/desktop\/web\/src\/(app|components|composer|inspector|settings|shell|sidebar|workspace)\//.test(
          rel,
        ),
    ],
  },
  {
    ...
    description:
      "Capability UI must not execute shell, browser automation, screenshot capture, writes, or live provider fetch",
    patterns: [
      ...
      { re: /getDisplayMedia\s*\(/gi, label: "screen capture" },
      {
        re: /writeFile|appendFile|unlink|rename|mkdir|rmdir|rm\s*\(/gi,
    ...
    blockWhen: [
      (rel) => /apps\/desktop\/web\/src\//.test(rel),
      (rel) =>
        /packages\/runtime\/src\/(?!mvp7-project-index|mvp9-terminal-policy|mvp10-terminal-policy|mvp10-scenarios|mvp13-|mvp14-|mvp15-)/.test(
          rel,
        ),
    ],
  },
  {
    ...
    patterns: [
      {
        re: /invoke\s*\(\s*["'](trust_native_project_root|cancel_native_project_scan|validate_native_project_root|scan_native_project_index|preview_native_project_file)/gi,
        label: "Tauri native FS command",
      },
      { re: /@tauri-apps\/api/gi, label: "Tauri API import" },
  ...
  },
  {
    ...
    allowWhen: [
      ...
      (rel) =>
        /packages\/runtime\/src\/(mvp9-|mvp5-scenarios|mvp7-project-index|mvp8-project-index|mvp10-scenarios)/.test(
          rel,
        ),
      (rel) => /apps\/desktop\/src-tauri\//.test(rel),
      (rel) => /docs\//.test(rel),
    ...
    blockWhen: [
      ...
      (rel) =>
        /packages\/runtime\/src\/(?!mvp9-|mvp5-scenarios|mvp7-project-index|mvp8-project-index|mvp14-)/.test(
          rel,
        ),
    ],
  },
  {
    ...
    blockWhen: [
      ...
      (rel) =>
        /packages\/runtime\/src\/(?!mvp8-project-index|mvp9-terminal-policy|mvp10-terminal-policy|mvp13-|mvp14-|mvp15-)/.test(
          rel,
        ),
    ],
  },
  ...
  },
  {
    ...
    blockWhen: [(rel) => /apps\/desktop\/web\/src\/(composer|settings|workspace)\//.test(rel)],
  },
  {
    id: "mvp11-ui-native-diagnostics-boundary",
    title: "MVP11 UI Native Diagnostics Boundary",
    description:
      "React UI must not directly import Tauri, Node fs/path, child_process, or native diagnostic capabilities",
    patterns: [
  ...
  },
  {
    ...
    description:
      "The only real terminal execution path is execute_terminal_command_real behind proposal approval and one-time token",
    patterns: [
      { re: /\bexecute_terminal_command\b/g, label: "old ambiguous execute_terminal_command" },
    ],
    allowWhen: [(rel) => /docs\//.test(rel), (rel) => /scripts\/side-effect-scan/.test(rel)],
    blockWhen: [
      (rel) => /apps\/desktop\/web\/src\//.test(rel),
      (rel) => /apps\/desktop\/src-tauri\/src\//.test(rel),
      (rel) => /packages\/runtime\/src\//.test(rel),
  ...
  },
  {
    ...
    description:
      "Diagnostic payloads must not leak raw home paths, raw file URLs, Bearer tokens, sk- keys, token=, api_key, or Authorization values",
    patterns: [
    ...
    allowWhen: [
      ...
      (rel) => /packages\/shared\/src\/project/.test(rel),
      (rel) =>
        /packages\/runtime\/src\/(secrets\/redaction|ue-diagnostics|mvp8-project-index|mvp10-scenarios)/.test(
          rel,
        ),
  ...
  },
  {
    ...
    description:
      "Diagnostics must not add automatic fixes, installs, commits, pushes, or default provider live network calls",
    patterns: [
      { re: /--fix/gi, label: "--fix" },
  ...
  },
  {
    ...
    description:
      "Controlled text mutation must stay in shared/runtime services, desktop runtime adapters, native bridge, docs, and tests",
    patterns: [
      {
        re: /apply_workspace_change|rollback_workspace_change|preview_workspace_change/gi,
        label: "native text mutation command",
      },
      { re: /applyChangeSet|rollbackChangeSet|previewChangeSet/gi, label: "change set UI action" },
      {
        re: /TextMutation|WorkspaceChangeSetV2|ChangeSetServiceV2/gi,
        label: "MVP12 text mutation type/service",
      },
    ],
    allowWhen: [
      (rel) => /packages\/(shared|runtime)\/src\//.test(rel),
  ...
  },
  {
    ...
    description:
      "Binary UE assets and executable/generated artifacts must remain blocked from text mutation",
    patterns: [
      {
        re: /\.(?:uasset|umap|ubulk|uexp|dll|exe)(?![A-Za-z0-9_])/gi,
        label: "blocked binary extension",
      },
    ...
    allowWhen: [
      ...
      (rel, line) =>
        /apps\/desktop\/web\/src\/inspector\/AssetMutationPanel/.test(rel) &&
        /executeMvp15AssetChangeSet/.test(line),
      (rel) => /docs\//.test(rel),
      (rel) => /\.test\./.test(rel),
      (rel) => /scripts\/side-effect-scan/.test(rel),
    ],
  ...
  },
  {
    ...
    patterns: [
      {
        re: /root_escape|stale_hash|trustedRootId|rootRelativePath/gi,
        label: "root containment or hash guard",
      },
    ],
    allowWhen: [
      (rel) => /packages\/(shared|runtime)\/src\//.test(rel),
    ...
    blockWhen: [
      (rel) =>
        /apps\/desktop\/web\/src\/(app|components|composer|inspector|settings|shell|workspace)\//.test(
          rel,
        ),
    ],
  },
  {
    ...
    description:
      "Session replay may show recorded summaries only and must not reapply preview/apply/rollback",
    patterns: [
      {
        re: /recordedOnlyActions|replaySafe|session_replay_never_reapplies/gi,
        label: "recorded-only replay marker",
      },
    ],
    allowWhen: [
      (rel) => /packages\/runtime\/src\//.test(rel),
      ...
      (rel) => /scripts\/side-effect-scan/.test(rel),
      (rel, line) =>
        /apps\/desktop\/web\/src\/inspector\/UtilityPlaceholderPanel/.test(rel) &&
        /recordedOnlyActions/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/inspector\/AssetMutationPanel/.test(rel) &&
        /recordedOnlyActions/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/inspector\/ChangesPanel/.test(rel) &&
        /replaySummary|recordedOnlyActions/.test(line),
    ],
    blockWhen: [
      (rel) =>
        /apps\/desktop\/web\/src\/(app|components|composer|inspector|settings|shell|sidebar|workspace)\//.test(
  ...
  },
  {
    ...
    description:
      "MVP12 repair and verification must not auto-install, auto-fix, commit, push, reset, checkout, clean, or create CI",
    patterns: [
  ...
  },
  {
    ...
    description:
      "Repair proposals must be deterministic and must not call live providers by default",
    patterns: [
      {
        re: /networkMode\s*:\s*["']live["']|fetch\s*\(|ProviderRunner|runProvider/gi,
        label: "provider live path",
      },
    ],
    allowWhen: [
      (rel) => /packages\/runtime\/src\/provider\//.test(rel),
      ...
      (rel) => /packages\/runtime\/src\/mvp15d-companion/.test(rel),
      (rel, line) =>
        /apps\/desktop\/web\/src\/runtime\/runtime-store/.test(rel) &&
        /Save All blocked/.test(line),
      (rel, line) =>
        /packages\/runtime\/src\/mvp14-(editor-observation-service|scenarios)/.test(rel) &&
        /Save All blocked/.test(line),
    ],
    blockWhen: [
      (rel) => /packages\/runtime\/src\/mvp12-change-set/.test(rel),
  ...
  },
  {
    ...
    description:
      "MCP remains read-only diagnostic context; mutating tools/call must not be introduced for repair apply",
    patterns: [
    ...
    allowWhen: [
      (rel) =>
        /packages\/(mcp-client|runtime)\/src\/(mcp-readonly|ue-diagnostics|runtime-router|agent-loop-runtime)/.test(
          rel,
        ),
  ...
  },
  {
    ...
    description:
      "Diff, evidence, audit, session, and status payloads must redact raw roots, home paths, secrets, and approval tokens",
    patterns: [
      { re: /[A-Za-z]:[\\/]+Users[\\/]+[^\\/\s"'`]+/g, label: "Windows home path" },
      { re: /\/Users\/[^/\s"'`]+/g, label: "macOS home path" },
      { re: /\/home\/[^/\s"'`]+/g, label: "Linux home path" },
  ...
  },
  {
    ...
    description:
      "React UI must not invoke UE Editor native commands directly; runtime adapters/services own that boundary",
    patterns: [
      {
        re: /@tauri-apps\/api|invoke\s*\(\s*["'](?:editor_|ue_editor|launch_editor|attach_editor)/gi,
        label: "direct native editor invoke",
      },
      {
        re: /attach_editor_session|launch_editor_session|execute_editor_operation/gi,
        label: "native editor command name",
      },
    ],
    allowWhen: [
      (rel) => /apps\/desktop\/web\/src\/runtime\//.test(rel),
  ...
  },
  {
    ...
    description:
      "MCP mutation pilot may classify and dry-run mutating tools, but must not add broad tools/call execution",
    patterns: [
    ...
    allowWhen: [
      ...
      (rel) =>
        /packages\/runtime\/src\/(mcp-readonly-runtime|ue-diagnostics|prompt\/context-pack|prompt\/prompt-builder)/.test(
          rel,
        ),
      (rel) => /packages\/runtime\/src\/provider\/mvp4-scenarios/.test(rel),
    ...
    blockWhen: [
      (rel) =>
        /apps\/desktop\/web\/src\/(app|components|composer|settings|shell|sidebar|workspace)\//.test(
          rel,
        ),
      (rel) =>
        /packages\/runtime\/src\/(?!mvp13-mcp-mutation|mvp13-dry-run-adapter|mvp13-scenarios|index|mcp-readonly-runtime|ue-diagnostics|prompt\/context-pack|prompt\/prompt-builder|provider\/mvp4-scenarios)/.test(
          rel,
        ),
    ],
  },
  {
    ...
    patterns: [
      {
        re: /save_asset|delete_asset|rename_asset|move_asset|compile_blueprint/gi,
        label: "asset write operation",
      },
      {
        re: /asset_mutation_blocked|blocked_asset_write/gi,
        label: "asset mutation blocked marker",
      },
    ],
    allowWhen: [
    ...
    blockWhen: [
      (rel) =>
        /apps\/desktop\/web\/src\/(app|components|composer|settings|shell|workspace)\//.test(rel),
    ],
  },
  {
    ...
    patterns: [
      {
        re: /Save\s+All|save_all|SavePackage|EditorAssetLibrary\.save|UEditorLoadingAndSavingUtils/gi,
  ...
  },
  {
    ...
    description:
      "Editor/MCP mutation pilot must not enable provider live mode by default or auto-apply provider output",
    patterns: [
      {
        re: /networkMode\s*:\s*["']live["']|autoApply|runProvider|ProviderRunner/gi,
        label: "provider live or auto apply",
      },
    ],
    allowWhen: [
      (rel) => /packages\/runtime\/src\/provider\//.test(rel),
  ...
  },
  {
    ...
    description:
      "Editor/MCP evidence and replay payloads must not store raw MCP args, secrets, tokens, or absolute paths",
    patterns: [
      {
        re: /rawArgs|raw_args|approval-token:|editor-approval-token:/gi,
        label: "raw args or token marker",
      },
      { re: /Bearer\s+[A-Za-z0-9._-]+|sk-[A-Za-z0-9][A-Za-z0-9._-]{7,}/g, label: "secret literal" },
      {
        re: /[A-Za-z]:[\\/]+Users[\\/]+[^\\/\s"'`]+|\/Users\/[^/\s"'`]+|\/home\/[^/\s"'`]+/g,
        label: "raw user path",
      },
    ],
    allowWhen: [
      (rel) =>
        /packages\/runtime\/src\/(mvp13-editor-operation-service|mvp13-mcp-mutation-service|mvp12-change-set)/.test(
          rel,
        ),
      (rel) => /apps\/desktop\/src-tauri\/src\/ue_editor/.test(rel),
      (rel) => /docs\//.test(rel),
  ...
  },
  {
    ...
    description:
      "Session replay may show editor/MCP/ChangeSet summaries only and must not execute operations, tools, apply, or rollback",
    patterns: [
    ...
    allowWhen: [
      ...
      (rel) => /scripts\/side-effect-scan/.test(rel),
      (rel, line) =>
        /packages\/shared\/src\/(ue-editor|session)/.test(rel) &&
        /replayOnly|recordedOnlyActions/.test(line),
      (rel, line) =>
        /packages\/runtime\/src\/mvp13-(editor-operation-service|editor-session|mcp-mutation-service)/.test(
          rel,
        ) && /replayOnly|recordedOnlyActions/.test(line),
      (rel, line) =>
        /packages\/runtime\/src\/mvp13-scenarios/.test(rel) &&
        /session-replay-only|approval-replay-blocked|mcp-replay-recorded/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/runtime\/runtime-store/.test(rel) && /replayOnly/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/stores\/ui-store/.test(rel) && /replayOnly:\s*true/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/inspector\/(EditorPanel|McpMutationPanel)/.test(rel) &&
        /replayOnly|Replay: recorded summaries only/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/inspector\/AssetMutationPanel/.test(rel) &&
        /replayOnly|Replay: recorded summaries only/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/inspector\/ChangesPanel/.test(rel) &&
        /replayOnly|recordedOnlyActions|recorded-only/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/inspector\/UtilityPlaceholderPanel/.test(rel) &&
        /recordedOnlyActions/.test(line),
      (rel, line) =>
        /apps\/desktop\/web\/src\/runtime\/editor-observation-native-adapter/.test(rel) &&
        /replayOnly/.test(line),
    ],
    blockWhen: [
      (rel) => /packages\/runtime\/src\/mvp13-/.test(rel),
  ...
  },
  {
    ...
    description:
      "Editor observation may stop UAgent sessions but must not kill, taskkill, or terminate UE processes",
    patterns: [
  ...
  },
  {
    ...
    patterns: [
      {
        re: /Command::new|cmd\s*\/c|powershell|sh\s+-c|shell\s*:\s*true/gi,
        label: "launch shell boundary",
      },
    ],
    allowWhen: [
      (rel, line) =>
        /apps\/desktop\/src-tauri\/src\/ue_editor_process/.test(rel) && /Command::new/.test(line),
      (rel) => /docs\//.test(rel),
      (rel) => /\.test\./.test(rel),
      (rel) => /scripts\/side-effect-scan/.test(rel),
      (rel) => /packages\/runtime\/src\/mvp10-terminal-policy/.test(rel),
  ...
  },
  {
    ...
    description:
      "MVP14 MCP adapters classify schemas and dry-run summaries but do not execute broad tools/call",
    patterns: [{ re: /tools\/call|callTool\s*\(/gi, label: "MCP tools/call" }],
    allowWhen: [
      (rel) =>
        /packages\/(mcp-client|runtime)\/src\/(mcp-readonly-runtime|runtime-router|agent-loop-runtime|ue-diagnostics|prompt\/)/.test(
          rel,
        ),
      (rel) => /apps\/desktop\/web\/src\/runtime\/desktop-runtime-adapter/.test(rel),
      (rel) => /docs\//.test(rel),
  ...
  },
  {
    ...
    description:
      "Observation payloads must not persist raw process args, raw executable paths, tokens, or secrets",
    patterns: [
      {
        re: /rawArgs|raw_args|rawExecutable|raw_executable|approval-token:|editor-approval-token:/gi,
        label: "raw process args or token",
      },
      { re: /Bearer\s+[A-Za-z0-9._-]+|sk-[A-Za-z0-9][A-Za-z0-9._-]{7,}/g, label: "secret literal" },
      {
        re: /[A-Za-z]:[\\/]+Users[\\/]+[^\\/\s"'`]+|\/Users\/[^/\s"'`]+|\/home\/[^/\s"'`]+/g,
    ...
    allowWhen: [
      ...
      (rel) => /scripts\/side-effect-scan/.test(rel),
      (rel) =>
        /packages\/runtime\/src\/(mvp12-change-set|mvp13-editor-operation-service|mvp13-mcp-mutation-service|mvp14-mcp-schema-adapters)/.test(
          rel,
        ),
      (rel) => /apps\/desktop\/src-tauri\/src\/ue_editor/.test(rel),
    ],
  ...
  },
  {
    ...
    description:
      "Replay may show recorded summaries only and must not attach, launch, execute, call MCP, apply, or rollback",
    patterns: [
      {
        re: /replay.*(?:attach|launch|execute|tools\/call|apply|rollback)/gi,
        label: "replay side effect marker",
      },
    ],
    allowWhen: [
      (rel) => /docs\//.test(rel),
      ...
      (rel, line) =>
        /packages\/runtime\/src\/mvp14-(editor-observation-service|scenarios)/.test(rel) &&
        /recordedOnlyActions|replay/.test(line),
    ],
    blockWhen: [
      (rel) => /packages\/runtime\/src\/mvp14-/.test(rel),
      (rel) => /apps\/desktop\/web\/src\//.test(rel),
  ...
  },
  {
    ...
    description:
      "Asset mutation must stay sandbox-only, exact-allowlisted, approval-bound, redacted, and replay-summary-only",
    patterns: [
      {
        re: /Save\s+All|save_all|SavePackage|EditorAssetLibrary\.save|UEditorLoadingAndSavingUtils/gi,
        label: "global editor save",
      },
      { re: /compile_blueprint|compile\s+blueprint/gi, label: "compile blueprint" },
      {
        re: /bulk_(?:delete|rename|save)|bulk\s+(?:delete|rename|save)/gi,
        label: "bulk asset operation",
      },
      {
        re: /\/Game\/(?!UAgentSandbox\b)[A-Za-z0-9_/.-]+|\/Content\/(?!UAgentSandbox\b)[A-Za-z0-9_/.-]+/g,
        label: "non-sandbox asset path",
      },
      { re: /tools\/call|callTool\s*\(/gi, label: "broad MCP tools/call" },
      {
        re: /@tauri-apps\/api|invoke\s*\(\s*["'](?:dry_run_asset_mutation|execute_asset_mutation|rollback_asset_mutation)/gi,
        label: "direct UI native asset invoke",
      },
      { re: /autoApply|provider.*apply|apply.*provider/gi, label: "provider auto apply" },
      {
        re: /rawArgs|rawCommandLine|approval-token:|asset-approval-token:|Bearer\s+[A-Za-z0-9._-]+|sk-[A-Za-z0-9][A-Za-z0-9._-]{7,}/gi,
        label: "raw args paths tokens",
      },
      {
        re: /if\s*\(\s*(?:approvalToken|token)\s*\)\s*(?:return\s+)?(?:accepted|true)|(?:approvalToken|token)\s*!==?\s*["']["'][^;\n]{0,80}(?:accepted|true)/gi,
        label: "non-empty token fake verification",
      },
      {
        re: /unknown[^;\n]{0,80}(?:accepted|executed|success|allow)|(?:accepted|executed|success|allow)[^;\n]{0,80}unknown/gi,
        label: "unknown result fail-open",
      },
      {
        re: /(?:audit|evidence|replay)[^;\n]{0,160}(?:rawProjectRoot|editorSessionId|pidHash|[A-Za-z]:[\\/]|\/home\/)|(?:rawProjectRoot|editorSessionId|pidHash|[A-Za-z]:[\\/]|\/home\/)[^;\n]{0,160}(?:audit|evidence|replay)/gi,
        label: "raw evidence identity or path",
      },
      {
        re: /replay.*(?:execute|dry[-_ ]?run|verify|rollback|tools\/call)/gi,
        label: "replay re-execute",
      },
      {
        re: /manifest[-_ ]?only.*real|real.*manifest[-_ ]?only/gi,
        label: "manifest-only real verification",
      },
  ...
  {
    id: "companion-plugin-provenance-boundary",
    title: "Companion Plugin Provenance Boundary",
    description:
      "Companion readiness must be tied to a validated manifest, clean source identity, installed modules, and loaded modules",
    patterns: [
      {
        re: /sourceCommit|sourceTreeSha256|manifestSha256(?!Prefix)|loadedModules|installedModules/gi,
        label: "companion provenance field",
      },
      {
        re: /dirty\s*[:=]\s*true|manifest[-_ ]only.*(?:ready|verified)/gi,
        label: "dirty or manifest-only provenance",
      },
    ],
    allowWhen: [
      (rel) => /packages\/runtime\/src\/mvp15d-companion/.test(rel),
  ...
  {
    id: "companion-generic-wrapper-boundary",
    title: "Companion Generic Wrapper Boundary",
    description:
      "Only attested exact-six mapping may call the companion; arbitrary method IDs and generic wrappers remain blocked",
    patterns: [
      {
        re: /generic\s+wrapper|methodId\s*[:=]|call_tool\s*\(/gi,
        label: "generic companion wrapper",
      },
      {
        re: /tools\/call[^;\n]{0,100}(?:method|operation|tool)/gi,
        label: "generic tools/call mapping",
      },
    ],
    allowWhen: [
      (rel) => /packages\/runtime\/src\/mvp15-/.test(rel),
  ...
  {
    id: "companion-saveall-bulk-boundary",
    title: "Companion Save-All / Bulk Boundary",
    description:
      "Companion writes are single-package and single-operation; Save All and bulk operations are forbidden",
    patterns: [
      {
        re: /Save\s+All|saveAll\s*[:=]\s*true|bulk_(?:delete|rename|save)|bulk\s+(?:delete|rename|save)/gi,
        label: "Save All or bulk mutation",
      },
    ],
    allowWhen: [
      (rel) => /packages\/runtime\/src\/mvp15d-companion/.test(rel),
  ...
  {
    id: "companion-non-sandbox-boundary",
    title: "Companion Non-Sandbox Boundary",
    description:
      "Companion asset paths must not broaden writes beyond the exact run root; Test01 is read-only source only",
    patterns: [
      {
        re: /\/Game\/(?!UAgentSandbox\b|Test01\b)[A-Za-z0-9_/.-]+/g,
        label: "non-sandbox companion asset path",
      },
      { re: /cross[-_ ]run|path traversal|overwrite/gi, label: "cross-run or traversal language" },
    ],
    allowWhen: [
      (rel) => /packages\/runtime\/src\/mvp15d-companion/.test(rel),
      (rel) => /packages\/runtime\/src\/mvp15-/.test(rel),
  ...
  {
    id: "companion-identity-leak-boundary",
    title: "Companion Identity Privacy Boundary",
    description:
      "UI and audit surfaces may publish only redacted identity summaries, never paths, PIDs, sessions, endpoints, or tokens",
    patterns: [
      {
        re: /companion[^;\n]{0,80}(?:raw|token|pid|session|endpoint|path)/gi,
        label: "raw companion identity field",
      },
      {
        re: /[A-Za-z]:[\\/]+Users[\\/]+[^\s"'`]+|\/home\/[^\s"'`]+/gi,
  ...
  {
    id: "companion-replay-boundary",
    title: "Companion Replay Boundary",
    description:
      "Replay is summary-only and cannot execute, verify, rollback, or call the companion again",
    patterns: [
      {
        re: /replay[^;\n]{0,120}(?:execute|verify|rollback|tools\/call|callTool)/gi,
        label: "companion replay side effect",
      },
    ],
    allowWhen: [
      (rel) => /packages\/runtime\/src\/mvp15-asset-replay/.test(rel),
      (rel) => /packages\/runtime\/src\/mvp15-asset-changeset/.test(rel),
      (rel) => /docs\//.test(rel),
      (rel) => /scripts\//.test(rel),
      (rel) => /\.test\./.test(rel),
    ],
...
const AUTHORITY_CATEGORIES = [
  {
    ...
    description:
      "Asset approval registration must resolve the native trusted-root registry and must not mint a caller-derived root identity",
    inspect(rel, source) {
      ...
      if (!/resolve_trusted_root_binding\s*\(/.test(body)) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "missing native trusted-root resolver",
            "registration does not call resolve_trusted_root_binding",
          ),
        );
      }
      if (/trusted-root:|canonicalize\s*\(\s*&?input\.trusted_project_root/.test(body)) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "caller-derived trusted-root identity",
            "registration canonicalizes or mints trust instead of resolving registry authority",
          ),
        );
      }
      return findings;
  ...
  },
  {
    ...
    description:
      "Registration and mutation guards must resolve the native observation/process registry rather than compare caller-declared session or PID duplicates",
    inspect(rel, source) {
      const findings = [];
      if (rel === "apps/desktop/src-tauri/src/asset_mutation.rs") {
        if (
          !/(?:resolve|validate)_asset_mutation_(?:observation|authority)(?:_binding)?(?:_at)?\s*\(/.test(
            source,
          )
        ) {
          findings.push(
            structuralFinding(
              this.id,
              rel,
              "missing native observation validator",
              "asset mutation does not call the ue_editor_process authority validator",
            ),
          );
        }
      }
      ...
      if (
        ...
        /observedEditorSessionId|observedPidHash|observed_editor_session_id|observed_pid_hash/.test(
          contractSource,
        )
      ) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "caller observation duplicate",
            "caller-declared observed session/PID remains in the production mutation contract",
          ),
        );
      }
      return findings;
  ...
  },
  {
    ...
    description:
      "Native registration and forward execution require the strict default-off UAGENT_ENABLE_ASSET_MUTATION=1 gate; caller booleans cannot enable it",
    inspect(rel, source) {
      const findings = [];
      if (rel === "apps/desktop/src-tauri/src/asset_mutation.rs") {
        if (
          !/UAGENT_ENABLE_ASSET_MUTATION/.test(source) ||
          !/(?:==|matches!)[^\n]{0,120}["']1["']/.test(source)
        ) {
          findings.push(
            structuralFinding(
              this.id,
              rel,
              "missing strict native gate",
              "native mutation source does not implement an exact value 1 environment gate",
            ),
          );
        }
      }
      if (
        ...
        /assetMutationGateEnabled|asset_mutation_gate_enabled/.test(source)
      ) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "caller mutation gate",
            "caller assetMutationGateEnabled remains in the production native-authority contract",
          ),
        );
      }
      return findings;
  ...
  },
  {
    ...
    description:
      "Every mutation phase must revalidate live authority and enforce absolute 15-minute forward and 20-minute recovery caps",
    inspect(rel, source) {
      if (rel !== "apps/desktop/src-tauri/src/asset_mutation.rs") return [];
      const findings = [];
      const hasCaps =
        /15\s*\*\s*(?:60\s*\*\s*1000|60_000)|900_000/.test(source) &&
      ...
      if (!hasCaps) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "missing absolute transaction/recovery caps",
            "15-minute forward and 20-minute recovery deadlines or stable reasons are absent",
          ),
        );
      }
      ...
      const guardBody = functionSlice(
        source,
        guardStart,
        /fn\s+record_asset_mutation_outcome_at\b/,
      );
      if (
        !/(?:revalidate|resolve|validate)_asset_mutation_(?:live_)?authority|(?:resolve|validate)_asset_mutation_observation/.test(
          guardBody,
        )
      ) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "missing per-guard live recheck",
            "authorize_asset_mutation_at does not call a live authority/observation revalidator",
          ),
        );
      }
      return findings;
  ...
  },
  {
    ...
    inspect(rel, source) {
      ...
      const findings = [];
      const addProject = functionSlice(
        source,
        /async\s+addProject\s*\(/,
        /async\s+confirmTrust\s*\(/,
      );
      if (/register(?:Mutation)?TrustedRootRef\s*\(|trustedRootRefs\.set\s*\(/.test(addProject)) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "pre-trust mutation root mapping",
            "addProject publishes a mutation-resolvable root before confirmTrust",
          ),
        );
      }
      const confirmTrust = functionSlice(
        source,
        /async\s+confirmTrust\s*\(/,
        /async\s+(?:removeProject|scanProject|scan)\s*\(/,
      );
      const trustIndex = confirmTrust.search(/trust_native_project_root/);
      const publishIndex = confirmTrust.search(
        /register(?:Mutation)?TrustedRootRef\s*\(|trustedRootRefs\.set\s*\(/,
      );
      if (publishIndex >= 0 && (trustIndex < 0 || publishIndex < trustIndex)) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "root mapping precedes native trust",
            "confirmTrust publishes the mapping before the native trust command",
          ),
        );
      }
      return findings;
  ...
  {
    id: "mvp15-ue-native-cleanup-boundary",
    title: "MVP15 UE Native Cleanup Boundary",
    description: "The companion plugin must never recursively delete a sandbox run directory",
    inspect(rel, source) {
      if (
        rel !==
        "integrations/unreal/UAgentAssetTools/Source/UAgentAssetTools/Private/UAgentAssetTool.cpp"
        ...
        return [];
      const findings = [];
      if (/DeleteDirectory\s*\([^;]*,\s*true\s*\)\s*;/.test(source)) {
        findings.push(
          structuralFinding(
            this.id,
            rel,
            "recursive DeleteDirectory",
            "UAgent companion cleanup passes Tree=true to DeleteDirectory",
          ),
        );
      }
      return findings;
...
function scanAuthorityFiles() {
  return MVP15_AUTHORITY_FILES.flatMap((relPath) => {
    ...
    if (!existsSync(absPath)) {
      return [
        structuralFinding(
          "mvp15-native-trust-authority-boundary",
          relPath,
          "authority source missing",
          "required production boundary file is missing",
        ),
      ];
    }
    try {
      ...
      return [
        structuralFinding(
          ...
          "required production boundary file could not be read",
        ),
      ];
    }
  });
    ...
    for (const pat of cat.patterns) {
      ...
      while ((match = pat.re.exec(content)) !== null) {
        ...
        findings.push({
          ...
          catTitle: cat.title,
          pattern: pat.label,
          severity: isAllowed(relPath, cat.allowWhen, lineText, pat)
            ? "ALLOWED"
            : isBlocked(relPath, cat.blockWhen)
              ? "BLOCKED"
              : "REVIEW",
          file: relPath,
          line: lineNum,
...
function runScanSelfTests() {
  ...
  if (
    !unsafeManifestOnly.some(
      (finding) =>
        finding.severity === "BLOCKED" && finding.pattern === "manifest-only real verification",
    )
  ) {
    throw new Error(
      "mvp15 asset mutation self-test did not block manifest-only real verification sample",
    );
  }
  ...
  if (!unsafeLivePayload.some((finding) => finding.severity === "BLOCKED")) {
    throw new Error(
      "mvp15 asset mutation self-test did not block live execute/rollback/Save All/mutation API sample",
    );
  }
  ...
  if (
    !unsafeNonEmptyToken.some(
      (finding) =>
        finding.severity === "BLOCKED" && finding.pattern === "non-empty token fake verification",
    )
  ) {
    throw new Error(
      "mvp15 asset mutation self-test did not block non-empty token fake verification",
    );
  }
  ...
  if (
    !unsafeUnknownResult.some(
      (finding) => finding.severity === "BLOCKED" && finding.pattern === "unknown result fail-open",
    )
  ) {
  ...
  if (
    !unsafeRawEvidence.some(
      (finding) =>
        finding.severity === "BLOCKED" && finding.pattern === "raw evidence identity or path",
    )
  ) {
  ...
  const authorityCases = [
    {
      ...
      unsafe:
        'fn register_asset_mutation_approval_at(input: Input) { let root = canonicalize(&input.trusted_project_root); let id = format!("trusted-root:{}", root); } fn authorize_asset_mutation_at() {}',
    ...
    },
    {
      ...
      unsafe:
        "export interface Guard { observedEditorSessionId: string; observedPidHash: string; }",
    ...
    },
    {
      ...
      unsafe:
        "async addProject(ref) { registerTrustedRootRef(ref, raw); } async confirmTrust(id) { await trust_native_project_root(id); } async removeProject() {}",
    ...
    {
      id: "mvp15-ue-native-cleanup-boundary",
      file: "integrations/unreal/UAgentAssetTools/Source/UAgentAssetTools/Private/UAgentAssetTool.cpp",
      unsafe: "IFileManager::Get().DeleteDirectory(*Directory, false, true);",
      safe: "IFileManager::Get().DeleteDirectory(*Directory, true, false);",
    },
  ];
